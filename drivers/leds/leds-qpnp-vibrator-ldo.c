...
inline int qpnp_vib_ldo_enable(struct vib_ldo_chip *chip, bool enable)
{
	...
	pr_err("Vibrator LDO %s \n",enable ? "enabled" : "disabled");//Bug 427130 caijiaqi.wt,ADD,20190121,P81081 add vibrator kernel log
...
static enum hrtimer_restart vib_overdrive_timer(struct hrtimer *timer)
{
	...
	pr_err("overdrive timer expired\n");//Bug 427130 caijiaqi.wt,ADD,20190121,P81081 add vibrator kernel log
	return HRTIMER_NORESTART;
}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	chip->state = val;
	pr_err("state = %d, time = %llums\n", chip->state, chip->vib_play_ms);//Bug 427130 caijiaqi.wt,ADD,20190121,P81081 add vibrator kernel log
	mutex_unlock(&chip->lock);
	schedule_work(&chip->vib_work);
