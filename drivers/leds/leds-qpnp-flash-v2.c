...
static int touch_num = 0; //+add by liuxingyu.wt , 2019/08/24 , modify touch level
static int qpnp_flash_led_switch_set(struct flash_switch_data *snode, bool on)
{
	...
	int rc, i, addr_offset;
	u8 val, mask;
	pr_err("liuxingyu qpnp_flash_led_switch_set 111\n");
	if (snode->enabled == on) {
		pr_debug("Switch node is already %s!\n",
			on ? "enabled" : "disabled");
		if(touch_num < 2){  //+add by liuxingyu.wt , 2019/08/24 , modify touch level
			touch_num++;
			return 0;
		}
	}
	pr_err("liuxingyu qpnp_flash_led_switch_set on:%d\n",on);
	if (!on) {
		rc = qpnp_flash_led_switch_disable(snode);
		return rc;
	}
	pr_err("liuxingyu qpnp_flash_led_switch_set 222\n");
