...
struct smb_charger {
	...
	struct notifier_block	otg_nb;		//bug433318, zhaobeilong@wt, 20180409, add usb disable node
#endif
	/* parallel charging */
...
//-bug 452108 ,caijiaqi.wt,Modify,20190618,some device use AFC charger ,charger error
	...
	bool			otg_present;
#ifdef CONFIG_USB_NOTIFIER
	bool			otg_block;		//bug433318, zhaobeilong@wt, 20180415, add usb disable node
#endif
	int			hw_max_icl_ua;
...
void smblib_notify_usb_host(struct smb_charger *chg, bool enable);	//bug433318, zhaobeilong@wt, 20180415, add usb disable node
