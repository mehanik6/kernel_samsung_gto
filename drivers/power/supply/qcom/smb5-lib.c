...
//bug433318, zhaobeilong@wt, 20180415, add usb disable node,start
...
irqreturn_t typec_state_change_irq_handler(int irq, void *data)
{
	...
	//bug440960, zhaobeilong@wt, 20180521, add usb disable node, start
	#ifdef CONFIG_USB_NOTIFIER
	u8 stat;
	int rc;
	#endif
	//bug440960, zhaobeilong@wt, 20180521, add usb disable node, end
...
irqreturn_t typec_attach_detach_irq_handler(int irq, void *data)
{
	...
	#ifdef CONFIG_USB_NOTIFIER
	chg->otg_block = 0;		//bug440960, zhaobeilong@wt, 20180521, add usb disable node, add
	#endif
...
#ifdef CONFIG_USB_NOTIFIER
	chg->otg_block = 0;
	if( otg && (chg->usb_psy->usb_host_flag) ) {
		chg->otg_block = 1;
		return;
	}
#endif
//bug433318, zhaobeilong@wt, 20180415, add usb disable node, end
...
//-bug 452108 ,caijiaqi.wt,Modify,20190618,some device use AFC charger ,charger error
	...
	smb5_get_adc_data(chip, USBIN_VOLTAGE, &val);
	usbin_v = val.intval;
	...
	usb_p = val.intval;
	smb5_get_adc_data(chip, USBIN_CURRENT, &val);
	usbin_i = val.intval;
	printk_counter++;
