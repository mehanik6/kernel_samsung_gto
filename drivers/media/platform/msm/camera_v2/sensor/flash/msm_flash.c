...
#define SS_FLASH_LEVEL  //+ add by liuxingyu.wt , 2019/08/08 , flash level
	...
	struct msm_flash_cfg_data_t *flash_data)
{
	...
	printk("liuxingyu msm_flash_off Enter\n");
	...
	uint32_t flash_data)
{
	...
	for (i = 0; i < flash_ctrl->torch_num_sources; i++) {
		if (flash_ctrl->torch_trigger[i]) {
			//printk("liuxingyu low_flash_current[%d] = %d", i, curr);
			led_trigger_event(flash_ctrl->torch_trigger[i],
				flash_data);
		}
	}
...
{
        printk("liuxingyu show_rear_flash");
        //return 0;
...
static ssize_t store_rear_flash(struct device *dev, struct device_attribute *attr, const char *buf, size_t cont)
{
	...
	printk("liuxingyu store_rear_flash : %d", temp);
//+yinjie1.wt,20190821,modify,for flash power dissipation
	if(temp == flash_level){
		printk("liuxingyu same brightness level");
		return cont;
	}
	...
	else if((temp == 1004) || (temp == 4)){
		msm_flash_level(my_flash_ctrl, 35);
		printk("liuxingyu 35");
	}
	else if((temp == 1006) || (temp == 6)){
		msm_flash_level(my_flash_ctrl, 50);
		printk("liuxingyu 50");
	}
	else if((temp == 1009) || (temp == 9)){
...
int samsung_flash_level_node_create(void)
{
        ...
        rc = device_register(dcam_device);*/
