...
#include <linux/hardware_info.h>//added by qihaiqian.wt for bug399604 on 20190218, add hardware info
/* Logging macro */
#undef CDBG
...
#define samsung_dcam_cal  //+ add by liuxingyu.wt , 2019/06/26 , dual camera
...
void msm_sensor_set_info(int event,void* buf)
{
   ...
   char* m_mod_qunhui = "qunhui";
    char* m_mod_dongci = "dongci";//Type Bug440446, wuhaopeng.wt add 2019.06.12. multi cam compatibility
   ...
   if(event < 0x10)
	...
	case 0x01:
		...
		break;
	case 0x02:
		hardwareinfo_set_prop(HARDWARE_BACK_WIDE_CAM, s_name);
		break;
	case 0x03:
		hardwareinfo_set_prop(HARDWARE_BACK_SUBCAM, s_name);
		break;
	...
	case 0x10:
		if((e_buf[0] == 0x55)&&(e_buf[1] == 0x19)){
           ...
           else if(msm_sensor_gc8034_otp_get_group_index(e_buf[(9*128 + 0x0378/8)]) == 2 && e_buf[(9*128 + 0x03C0/8)] == 0x0A)
           {
            ...
            else if((e_buf[0] == 0x55)&&(e_buf[1] == 0x13))
            {
		   	hardwareinfo_set_prop(HARDWARE_BACK_CAM_MOUDULE_ID,m_mod_dongci);
			s_name = m_mod_dongci;
			cam_num = 3;
	  }
			...
			(struct msm_sensor_ctrl_t *s_ctrl)
{
	int32_t rc = 0;
	uint32_t session_id = 0;
	pr_err("start to  create_v4l_subdev,line=%d,rc=%d",__LINE__,rc);
	if (s_ctrl->bypass_video_node_creation == 0) {
		pr_err(" create_v4l_subdev,line=%d,rc=%d",__LINE__,rc);
	...
	pr_err("end  create_v4l_subdev,line=%d,rc=%d",__LINE__,rc);
	return rc;
}
...
//+Type Bug440446, wuhaopeng.wt, add, 2019.06.12, multi cam compatibility
#if 1
...
#ifdef HI1336_BACKMAIN_MATCH_MODULE_INFO     //add by shanglei.wt  for N9 bringup
...
{
	uint16_t s5k3l6_addr_temp = 0x00;  //Type Bug440446, wuhaopeng.wt, add, 2019.06.12, multi cam compatibility
...
CSID_TG:
	...
	if (rc < 0) {
      ...
      {
          if(hi846_insensor_read_otp_info_match_sensor(s_ctrl) == false)     //add for N9 bringup for hi846 
          {
	...
	msm_sensor_fill_sensor_info(s_ctrl, probed_info, entity_name);
      ...
      msm_sensor_set_info((slave_info->camera_id),(void*)slave_info->sensor_name);//add forset sensor information
	//added by qihaiqian.wt for bug399604 on 20190218, add hardware info.
...
/*static ssize_t store_dcam_cal(struct device *dev, struct device_attribute *attr, char *buf)
{
	//printk("liuxingyu store_dcam_cal");
	return 0;
}*/
...
int samsung_dcam_cal_node_create(void)
{
    ...
    /*dcam_device->init_name = "dcam";
    dcam_device->class = dcam_class;
    dcam_device->release = (void (*)(struct device *))kfree;
	rc_1 = device_register(dcam_device);*/
