...
extern void msm_sensor_set_info(int event,void* buf);//add for set sensor information by shanglei.wt
	...
	for (j = 0; j < eeprom_map_array->msm_size_of_max_mappings; j++) {
		...
		for (i = 0; i < eeprom_map->memory_map_size; i++) {
			if (eeprom_map->mem_settings[i].i2c_operation ==
				...
				MSM_CAM_SINGLE_LOOP_READ) {    //added by qihaiqian.wt for bug399604 on 20190213, add loop read ops.
	...
	struct msm_eeprom_memory_map_array *eeprom_map_array)
{
	...
	for (j = 0; j < eeprom_map_array->msm_size_of_max_mappings; j++) {
		...
		for (i = 0; i < eeprom_map->memory_map_size; i++) {
			...
			break;
			case MSM_CAM_READ: {
                           uint16_t m; //for hi556 read otp
                           uint16_t read_val = 0;//for hi556 read otp
				e_ctrl->i2c_client.addr_type =
					eeprom_map->mem_settings[i].addr_type;
				if((e_ctrl->cci_master == 1)&&(eeprom_map->mem_settings[i].reg_addr == 0x0108)){//for hi556 read otp
			...
			break;
			 //added by qihaiqian.wt for bug399604 on 20190213, add loop read ops.
	...
	memptr = e_ctrl->cal_data.mapdata;
       msm_sensor_set_info((e_ctrl->subdev_id+0x10),(void*)memptr);//add forset sensor information
