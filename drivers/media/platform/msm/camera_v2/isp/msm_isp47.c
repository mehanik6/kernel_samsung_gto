...
int msm_vfe47_set_clk_rate(struct vfe_device *vfe_dev, long *rate)
{
	...
	*rate = 465000000;  //add by liuxingyu.wt , 2019/07/31 , sof freeze
	pr_err("liuxingyu 1 vfe clock is %ld",*rate);
	clk_rate = clk_round_rate(vfe_dev->vfe_clk[clk_idx], *rate);
	if (vfe_dev->vfe_clk_info[clk_idx].clk_rate == clk_rate)
		return rc;
         pr_err("liuxingyu 2 vfe clock is %ld",*rate);
	prev_clk_rate =
	...
	/*set vfe clock*/
	pr_err("liuxingyu 3 vfe clock is %ld",*rate);
