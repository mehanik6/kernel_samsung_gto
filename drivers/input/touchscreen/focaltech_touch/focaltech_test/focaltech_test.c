 ...
 */
static int fts_test_get_testparams(const struct firmware *fw)
{
    int ret = 0;

    ret = fts_test_get_testparam_from_ini(fw);
 ...
 *
 */
static int fts_test_entry(const struct firmware *fw)
{
    int ret = 0;
    ...
    ret = fts_test_get_testparams(fw);
    if (ret < 0) {
...
static void waite_for_fw_upgrading(void)
{
    int vid=0;
    if (fts_data->fw_loading == 1)
    {
        for(;fts_data->fw_loading == 1;){
